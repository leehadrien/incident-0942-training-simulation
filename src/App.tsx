...
const guideScripts: Record<Phase | 'briefing' | 'tour' | 'dashboard', string> = {
  ...
  tour: 'Here is the actual interface you will use. Start with the Live Incident Story so the numbers have context. Next, scan Live Metrics for what changed. Then click Evidence cards and read the service topology to see where the failure is spreading. Mara will glow the area she is explaining and translate the technical signal into plain language. When Signals reaches the target, move to the decision cards and choose the best supported action.',
...
const tourSteps: { focus: TourFocus; caption: string }[] = [
  { focus: 'story', caption: 'Start with the Live Incident Story. This tells you what happened before you look at the numbers.' },
  { focus: 'metrics', caption: 'Next, scan Live Metrics. Red means customer impact. Amber means something is getting worse.' },
  { focus: 'evidence', caption: 'Then click Evidence cards. These are the clues that explain why the numbers changed.' },
  { focus: 'topology', caption: 'Use the Service Topology to see how one failing service can affect the rest of the system.' },
  { focus: 'coach', caption: 'Mara explains the technical signal in plain language and tells you what to do next.' },
  { focus: 'decisions', caption: 'When Signals reaches the target, use the Decision Cards. Pick the action best supported by the evidence.' },
]

const guidedPhaseIntros: Record<Phase, string> = {
  diagnose: 'Step one. Diagnose means find where the problem started. Click evidence boxes until Signals reaches the target, then choose the cause.',
  contain: 'Nice. Step two is containment. That means stop the problem from spreading. Look at the service map, then choose the action that blocks more failed requests.',
  recover: 'Good. Step three is recovery. That means return the system to the last working version. Use the change history and choose the safest recovery action.',
  communicate: 'Great. Step four is communication. That means tell the team what happened, what you did, what proves recovery, and what you will check next.',
...
function NarrationButton({ text, audioKey, label = 'LISTEN TO GUIDE', onCaptionChange, onPlayingChange }: { text: string; audioKey: string; label?: string; onCaptionChange?: (caption: string | null) => void; onPlayingChange?: (playing: boolean) => void }) {
  ...
  const toggle = () => {
    ...
    const audioUrl = guideAudioUrls[audioKey]
    if (!audioUrl) {
      playFallback()
      return
    }
