...
const guideScripts: Record<Phase | 'briefing' | 'tour' | 'dashboard', string> = {
  ...
  tour: 'Here is the interface you will use. Start on the left rail. That is where the mission brief and Mara coaching live. Next, look to the right rail. Live Signals shows what changed, and Evidence gives you the clues behind those changes. Then use the center service map to understand how Checkout connects to Inventory, Payment, Cart, and the queue. Last, look at the four answer cards along the bottom. Those are the choices you will work through after you inspect enough signals. The order can change, so read the evidence and choose the action that is best supported.',
...
const tourSteps: { focus: TourFocus; caption: string }[] = [
  ...
  { focus: 'decisions', caption: 'These four answer cards are what you will work through. Read each option, then choose the action best supported by evidence.' },
]
...
function Simulation() {
  ...
  }, [briefingOpen, tourOpen, textSize, captionsEnabled, phase, selectedEvidence.length])
  ...
  return (
    <main className="simulation-shell" data-text-size={textSize}>
        ...
        <div className="briefing-backdrop" role="presentation">
          <section className="briefing-modal" role="dialog" aria-modal="true" aria-labelledby="briefing-title" aria-describedby="briefing-summary">
            ...
            <div className="briefing-hero">
              ...
              <div className="starter-instruction" role="note">
                <span>NEXT STEP</span>
                <strong>Choose your difficulty, set your accessibility options, then click Begin Interface Tour or Skip Tour.</strong>
              </div>
            ...
            <section className="briefing-three-panel" aria-label="Training overview">
              <article>
                <span>01 · SCENARIO BACKGROUND</span>
                <strong>A normal shopping morning became a customer-impacting incident.</strong>
                <p>Relay Nine Commerce needs you to follow the evidence, identify where checkout is failing, and choose the safest action to restore service.</p>
              </article>
              <article>
                <span>02 · DIFFICULTY PATH</span>
                <strong>Pick how much guidance you want before the timer starts.</strong>
                <p>Guided explains more in plain language. Standard gives balanced support. Challenge uses more technical language and fewer prompts.</p>
              </article>
              <article>
                <span>03 · ACCESSIBILITY AND START</span>
                <strong>Set text size, captions, and background music before you begin.</strong>
                <p>Use captions for narration, the A buttons for text size, and the speaker control to turn background music on or off.</p>
              </article>
            </section>
            ...
            <div className="briefing-controls">
              ...
              <button className={`music-toggle ${musicEnabled ? 'active' : ''}`} onClick={() => setMusicEnabled((enabled) => !enabled)} aria-pressed={musicEnabled}>
                <i className="speaker-glyph" aria-hidden="true">●</i><span>BACKGROUND MUSIC</span><strong>{musicEnabled ? 'ON' : 'OFF'}</strong>
              </button>
              <button className={`captions-toggle ${captionsEnabled ? 'active' : ''}`} onClick={() => setCaptionsEnabled((enabled) => !enabled)} aria-pressed={captionsEnabled}><strong>CC</strong><span>{captionsEnabled ? 'CAPTIONS ON' : 'CAPTIONS OFF'}</span></button>
              <button className="secondary-button skip-tour-button" onClick={beginMission}>SKIP TOUR · START CHALLENGE</button>
              <button className="begin-button" onClick={continueFromBriefing}>{missionStarted ? 'RETURN TO INCIDENT' : 'BEGIN INTERFACE TOUR'} <span>▶</span></button>
            </div>
      ...
      {tourOpen && (
        <div className="briefing-backdrop tour-backdrop" role="presentation">
          <section className="tour-modal" role="dialog" aria-modal="true" aria-labelledby="tour-title">
            ...
            <div className="tour-heading">
              ...
              <p>Press play. Iris will walk you through the real interface, and each area she describes will glow on the screen behind this panel.</p>
              <NarrationButton
                text={guideScripts.tour}
                audioKey="tour-guide"
                label="PLAY VOICEOVER TOUR"
